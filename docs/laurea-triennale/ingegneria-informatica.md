...
Questo file contiene le testimonianze degli esami orali di vari studenti del corso di laurea in **Ingegneria Informatica Laurea Triennale** all' **Unical** ( _Università della Calabria_ ) e fa parte del progetto [Indice Argomenti Orali](https://github.com/UnicalLoveTelegram/IndiceArgomentiOrale) gestito dall'organizzazione **UnicalLoveTelegram**
...
- [Indice delle domande degli esami orali: Ingegneria Informatica LT](#indice-delle-domande-degli-esami-orali-ingegneria-informatica-lt)
    ...
    - [Qualità del servizio](#qualita-del-servizio)
        - [Amedeo](#amedeo)
...
- Anonimi
    ...
    - `char *f(char*d, const char *s){ char*p=d; while(*p) p++; while(*p++ = *s++); return d; }`
...
- Andrea
    - calcolo del round trip time (ricorsione) e perché è utile
    - query flooding pro e contro e applicazioni
    - protocolli P2P
    - chord, funzionamento e dimensione delle finger tablet
    - Skype, approccio del supernodo
...
- Anonimo 1
    - verifica palindroma ricorsiva
    - differenza tra gaus diagonale e gaus determinante
    - differenza tra file tipati e file di testo
    - differenza tra heap e abr
    - quantificare gli spostamenti quando si aggiunge un nodo con ultimo livello pieno su heap
- Anonimo 2
    - valutare espressione con delle regex
    - definizione di backtracking
    - elencare i metodi di visita di un abr
    - scrivere metodo visita per livelli di un abr
    - complessità del merge sort
- Anonimo 3
    - Scrivere costruttore classe Heap ed il metodo add
    - scrivere il metodo heapsort
    - elencare i metodi di random access file
    - che vuol dire classe base
- Anonimo 4
    - lista concatenata semplice in veste ricorsiva
    - complessità di selection e bubble sort
- Anonimo 5
    - aggiunta elemento su abr con ricorsione
    - elencare proprietà dei set
    - calcolare indice lista tramite hashcode
    - liste di trabocco hashset
- Anonimo 5
    - definizione di type erasure
    - metodo generico ricorsivo che inserisce un elemento su una linked list mantenendo la lista ordinata usando listiterator
    - risolvere problema permutazioni, differenza tra permutazioni e disposizioni
- Anonimo 6
    - ordinare linkedlist con bubble sort
    - serializzazione
- Anonimo 7
...
- Giuseppe Magliano
    - Illustrare il concetto di potenziale elettrico differenziale e derivare da esso la legge di Kirchoff per le tensioni
    - Illustrare la legge di Ohm
    - Illustrare il fenomeno di polarizzazione dei materiali
    - Illustrare il principio di sovrapposizione degli effetti, descrivendone altresı́ un esempio applicativo
